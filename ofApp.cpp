		...
		for (float phi_deg = 0; phi_deg < 360; phi_deg += phi_deg_step) {
			...
			for (float theta_deg = 0; theta_deg <= 180; theta_deg += theta_deg_step) {
				...
				auto noise_value = ofNoise(
					cos(phi_deg * DEG_TO_RAD) * 8,
					sin(phi_deg * DEG_TO_RAD) * 8,
					cos(theta_deg * DEG_TO_RAD) * 8,
				...
				auto noise_value_1 = ofNoise(
					cos(phi_deg * DEG_TO_RAD) * 8,
					sin(phi_deg * DEG_TO_RAD) * 8,
					cos((theta_deg - theta_deg_step) * DEG_TO_RAD) * 8,
					radius * 0.01 + ofGetFrameNum() * 0.04);
				auto noise_value_2 = ofNoise(
					cos((phi_deg + phi_deg_step) * DEG_TO_RAD) * 8,
					sin((phi_deg + phi_deg_step) * DEG_TO_RAD) * 8,
					cos(theta_deg * DEG_TO_RAD) * 8,
					radius * 0.01 + ofGetFrameNum() * 0.04);
